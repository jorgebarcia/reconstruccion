...
class Reconstruccion:
    def __init__(self, datos):
        ...
        # self.gradient_accuracy_threshold = 0.1  # Definir un umbral adecuado para la discrepancia del gradiente
        # self.test_simplified_gradient()
        # self.integracion_variacional()
        # self.test_gradient()
        self.test_gradient()
        # self.test_calculate_gradients()
        # self.test_energy_function()
        self.test_gradient_over_range()
        self.integracion_variacional()
    ...
    def normalize_data(self, data):
        ...
        print(
            f"Data stats after normalization: min={normalized_data.min()}, max={normalized_data.max()}, mean={normalized_data.mean():.5f}, std={normalized_data.std():.5f}")
        return normalized_data

    def calcular_gradientes(self, eps=1e-3):
        i_a = self.datos.img_dict['right'].astype(np.float32)
        i_b = self.datos.img_dict['left'].astype(np.float32)
        i_c = self.datos.img_dict['top'].astype(np.float32)
        i_d = self.datos.img_dict['bottom'].astype(np.float32)
        ...
        print("s_dx stats before normalization:", np.min(s_dx), np.max(s_dx), np.mean(s_dx), np.std(s_dx))
        print("s_dy stats before normalization:", np.min(s_dy), np.max(s_dy), np.mean(s_dy), np.std(s_dy))

        s_dx = self.normalize_data(s_dx)
        s_dy = self.normalize_data(s_dy)

        return s_dx, s_dy

    def energy_function(self, z_flat, s_dx, s_dy, lambda_reg):
    ...
    def gradient_function(self, z_flat, s_dx, s_dy, lambda_reg):
        z = z_flat.reshape(s_dx.shape)
        dz_dx = np.gradient(z, axis=1)
        dz_dy = np.gradient(z, axis=0)
        ...
        grad_total = np.gradient(grad_data_fidelity_x, axis=1) + np.gradient(grad_data_fidelity_y,
                                                                             axis=0) + grad_regularization
        return grad_total.flatten()

    def integracion_variacional(self):
        s_dx, s_dy = self.calcular_gradientes()
        z0 = np.zeros_like(s_dx)
        result = minimize(self.energy_function, z0.flatten(), args=(s_dx, s_dy, 1.0), method='L-BFGS-B',
                          jac=self.gradient_function)
        if result.success:
            self.z = result.x.reshape(s_dx.shape)
            print("Optimización exitosa.")
        else:
            print("Falló la optimización.")
            raise RuntimeError("La optimización no convergió")
    ...
    def test_gradient(self):
        ...
        # Calcula el gradiente analítico usando tu método existente.
        analytical_grad = self.gradient_function(z_flat_test, s_dx_test, s_dy_test, lambda_reg)

        # Calcula el gradiente numérico usando aproximación.
        numerical_grad = approx_fprime(z_flat_test, lambda z: self.energy_function(z, s_dx_test, s_dy_test, lambda_reg),
                                       epsilon=np.sqrt(np.finfo(float).eps))

        # Compara los gradientes calculados y numéricos.
        discrepancy = np.linalg.norm(analytical_grad - numerical_grad) / (
                    np.linalg.norm(analytical_grad) + np.linalg.norm(numerical_grad))
        print(f"Discrepancia en el gradiente: {discrepancy}")

        # Ajusta la tolerancia aquí.
        assert np.allclose(analytical_grad, numerical_grad, atol=0.2), "La verificación del gradiente falló"

    def test_calculate_gradients(self):
        calculated_s_dx, calculated_s_dy = self.calcular_gradientes()
        expected_s_dx = np.gradient(self.some_known_good_output,
                                    axis=1)  # Aquí deberías tener una forma de calcular un buen resultado esperado
        expected_s_dy = np.gradient(self.some_known_good_output, axis=0)

        assert np.allclose(calculated_s_dx, expected_s_dx, rtol=1e-3, atol=1e-3), "s_dx calculation error"
        assert np.allclose(calculated_s_dy, expected_s_dy, rtol=1e-3, atol=1e-3), "s_dy calculation error"

    def test_energy_function(self):
        z_test = np.array([[0, 1], [1, 0]], dtype=np.float32).flatten()  # Superficie simple
        s_dx_test, s_dy_test = np.gradient(z_test.reshape(2, 2))  # Gradientes conocidos
        expected_energy = np.sum(
            (s_dx_test - s_dx_test) ** 2 + (s_dy_test - s_dy_test) ** 2)  # Energía esperada, debería ser 0
        calculated_energy = self.energy_function(z_test, s_dx_test, s_dy_test, 0.1)
        assert np.isclose(calculated_energy,
                          expected_energy), "Error en energy_function: Energía calculada no es la esperada"
        print("Prueba de energy_function pasada.")

    def test_gradient_over_range(self):
        for scale in np.linspace(0.1, 2.0, 20):
            z_test = np.random.rand(10, 10) * scale
            s_dx_test, s_dy_test = np.gradient(z_test)
            z_flat_test = z_test.flatten()
            lambda_reg_test = 0.1

            analytical_grad = self.gradient_function(z_flat_test, s_dx_test, s_dy_test, lambda_reg_test)
            numerical_grad = approx_fprime(z_flat_test,
                                           lambda z: self.energy_function(z, s_dx_test, s_dy_test, lambda_reg_test),
                                           np.sqrt(np.finfo(float).eps))

            discrepancy = np.linalg.norm(analytical_grad - numerical_grad) / (
                        np.linalg.norm(analytical_grad) + np.linalg.norm(numerical_grad))
            print(f"Scale {scale:.2f} - Gradient discrepancy: {discrepancy}")
    ...
    def plot_superficie(self, ver_textura=True):
        ...
        if ver_textura and self.datos.textura is not None:
            ...
            axis_2.plot_surface(x * self.dpixel, y * self.dpixel, self.z, facecolors=self.datos.textura / 255.0,
                                shade=False)
    ...
    def histogramear(self):
        ...
        for i, (key, image) in enumerate(self.datos.img_dict.items()):
            ...
            if key != 'textura':
                ...
                ax_hist.tick_params(axis='both', which='major', labelsize=4)
                # ax_hist.grid(False)
                ax_hist.set_title(f'Histograma {key}', fontsize=7)
                ax_hist.set_xlabel('Valores', fontsize=5)
                ax_hist.set_ylabel('Frecuencia', fontsize=5)
                ax_hist.grid(False)
                ...
                ax_stats.text(0.5, 0.5, stats_txt, horizontalalignment='center', verticalalignment='center',
                              fontsize=12)
                ax_stats.axis('off')
...
# plt.plot(ax_x, perfil_06, label='Sin Mecanizado')
# plt.plot(ax_x, perfil_06M, 'r', label='Mecanizada')
# plt.legend()
