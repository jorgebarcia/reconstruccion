...
class reco_superficie3d:
    def __init__(self, img_rutas, dpixel=1 / 251.8750):
        ...
        # self.aplanacion()
        self.procesar_y_actualizar_imagenes(max_depth=4, min_samples_leaf=20, sigma=1)
    ...
    def joder(self,image):
        y_index,x_index=image.shape

    def calcular_tamaño_bloque_optimo(self, alto, ancho):
        # Encuentra el mayor divisor común para alto y ancho
        return gcd(alto, ancho)

    # Asegúrate de que este método está definido en la clase
    def dividir_imagen_en_bloques_optimos(self, image):
        alto, ancho = image.shape
        tamaño_bloque = self.calcular_tamaño_bloque_optimo(alto, ancho)

        bloques = []
        coordenadas = []
        for y in range(0, alto, tamaño_bloque):
            for x in range(0, ancho, tamaño_bloque):
                bloque = image[y:y + tamaño_bloque, x:x + tamaño_bloque]
                bloques.append(bloque)
                coordenadas.append((y, x))
        return bloques, coordenadas
    def aplicar_ajuste_polinomico_a_bloque(self, bloque, max_depth=4, min_samples_leaf=20):
        # ... Este método podría renombrarse para reflejar que no solo hace ajuste polinómico ...
        y, x = np.indices(bloque.shape)
        x_flat = x.ravel()
        y_flat = y.ravel()
        z = bloque.ravel()

        # Usamos las coordenadas x e y como variables independientes
        ...
        # Calculamos el R^2 para evaluar el ajuste
        r2 = r2_score(z, z_pred)
        print(f"R^2 del ajuste con árbol de decisión: {r2:.3f} - {'Bueno' if r2 > 0.5 else 'Malo'}")

        return modelo, r2

    def reconstruir_imagen_desde_modelos(self, modelos, coordenadas, shape_imagen):
        # Este método reconstruirá la imagen completa a partir de los modelos ajustados a los bloques
        imagen_reconstruida = np.zeros(shape_imagen)
        for ((modelo, _), (y_inicio, x_inicio)) in zip(modelos, coordenadas):
            # Generamos las coordenadas del bloque a reconstruir
            y_fin = min(y_inicio + modelo.tree_.max_depth, shape_imagen[0])
            x_fin = min(x_inicio + modelo.tree_.max_depth, shape_imagen[1])
            y, x = np.mgrid[y_inicio:y_fin, x_inicio:x_fin]
            y_flat = y.ravel()
            x_flat = x.ravel()

            # Usamos el modelo para predecir los valores en el bloque
            X_pred = np.column_stack([x_flat, y_flat])
            z_pred = modelo.predict(X_pred).reshape(y_fin - y_inicio, x_fin - x_inicio)

            # Colocamos los valores predichos en su lugar correspondiente en la imagen
            imagen_reconstruida[y_inicio:y_fin, x_inicio:x_fin] = z_pred
            imagen_suavizada: object = gaussian_filter(imagen_reconstruida, sigma=5)
        # return imagen_reconstruida
        return imagen_suavizada

    def procesar_y_actualizar_imagenes(self, max_depth=4, min_samples_leaf=20, sigma=1):
        for key, image in self.img_dict.items():
            if key != 'textura':
                bloques, coordenadas = self.dividir_imagen_en_bloques_optimos(image)
                modelos = [self.aplicar_ajuste_polinomico_a_bloque(bloque) for bloque in bloques]
                imagen_ajustada = self.reconstruir_imagen_desde_modelos(modelos, coordenadas, image.shape)
                self.img_dict[key] = imagen_ajustada
    # def calcular_tamaño_bloque_optimo(self, alto, ancho):
    #     # Encuentra el mayor divisor común para alto y ancho
    #     return gcd(alto, ancho)
    #
    ...
    #     return bloques, coordenadas
    #
    # def aplicar_ajuste_polinomico_a_bloque(self, bloque, grado=3):
    #     y, x = np.indices(bloque.shape)
    #     x_flat = x.ravel()
    #     y_flat = y.ravel()
    #     z = bloque.ravel()
    ...
    #
    #     r2 = r2_score(z, z_pred)
    #     print(f"R^2 del ajuste: {r2:.3f} - {'Bueno' if r2 > 0.5 else 'Malo'}")
    #
    #     # Devuelve el modelo y el score R^2 como una tupla
    #     return (modelo, r2)
    #
    # def reconstruir_imagen_desde_modelos(self, modelos, coordenadas, tamaño_bloque, shape_imagen):
    #     imagen_reconstruida = np.zeros(shape_imagen)
    #
    #     for ((modelo, r2), (y_inicio, x_inicio)) in zip(modelos, coordenadas):
    #         # Aquí asumimos que el tamaño del bloque es uniforme para simplificar
    #         # la reconstrucción. Si el último bloque es más pequeño, este enfoque
    #         # necesita ajustes para manejar esos casos.
    #         y, x = np.mgrid[y_inicio:y_inicio + tamaño_bloque, x_inicio:x_inicio + tamaño_bloque]
    #         y_flat = y.ravel()
    #         x_flat = x.ravel()
    #
    #         X = np.column_stack([x_flat, y_flat])
    ...
    #         X_poly = poly.fit_transform(X)
    #
    #         z_pred = modelo.predict(X_poly).reshape(tamaño_bloque, tamaño_bloque)
    #
    #         imagen_reconstruida[y_inicio:y_inicio + tamaño_bloque, x_inicio:x_inicio + tamaño_bloque] = z_pred[
    #                                                                                                     :shape_imagen[
    #                                                                                                          0] - y_inicio,
    #                                                                                                     :shape_imagen[
    #                                                                                                          1] - x_inicio]
    ...
    #     return imagen_reconstruida
    #
    # def procesar_y_actualizar_imagenes(self, grado=2):
    #     for key, image in self.img_dict.items():
    #         if key != 'textura':
    #             bloques, coordenadas = self.dividir_imagen_en_bloques_optimos(image)
    #             modelos = [self.aplicar_ajuste_polinomico_a_bloque(bloque, grado) for bloque in bloques]
    #             tamaño_bloque = self.calcular_tamaño_bloque_optimo(*image.shape[:2])
    #             imagen_ajustada = self.reconstruir_imagen_desde_modelos(modelos, coordenadas, tamaño_bloque,
    #                                                                     image.shape)
    #             self.img_dict[key] = imagen_ajustada
