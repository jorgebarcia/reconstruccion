...
class Reconstruccion:
    ...
    def integracion_variacional(self):
        # Asumiendo que calcular_gradientes y otros métodos están definidos
        s_dx, s_dy = self.calcular_gradientes()
        z0 = np.zeros_like(s_dx)

        # Realiza una prueba de gradiente antes de la optimización
        self.test_gradient_over_range()  # Verifica la precisión de los gradientes

        # Procede con la optimización después de verificar los gradientes
        result = minimize(self.energy_function, z0.flatten(), args=(s_dx, s_dy, 1.0), method='L-BFGS-B', jac=self.gradient_function)
        if result.success:
    ...
    def test_gradient_over_range(self):
        # Testea el cálculo del gradiente sobre un rango de puntos
        for scale in np.linspace(0.1, 2.0, 20):  # Escalas diferentes para probar
    ...
    def test_gradient(self):
        # Crear un pequeño conjunto de datos para prueba
        z_test = np.random.rand(10, 10)
        s_dx_test, s_dy_test = np.gradient(z_test)
        z_flat_test = z_test.flatten()
        lambda_reg_test = 0.1

        # Calcula el gradiente usando la función analítica
        analytical_grad = self.gradient_function(z_flat_test, s_dx_test, s_dy_test, lambda_reg_test)

        # Calcula el gradiente de forma numérica
        from scipy.optimize import approx_fprime
        numerical_grad = approx_fprime(z_flat_test,
                                       lambda z: self.energy_function(z, s_dx_test, s_dy_test, lambda_reg_test),
                                       np.sqrt(np.finfo(float).eps))

        # Compara los gradientes
        discrepancy = np.linalg.norm(analytical_grad - numerical_grad)
        print("Discrepancy in the gradient:", discrepancy)

    def test_simplified_gradient(self):
        z_test = np.random.rand(10, 10)
        s_dx_test, s_dy_test = np.gradient(z_test)
        z_flat_test = z_test.flatten()
        lambda_reg_test = 0.1
        ...
        def func(z_flat):
            return self.energy_function(z_flat, s_dx_test, s_dy_test, lambda_reg_test)

        def grad(z_flat):
            return self.gradient_function(z_flat, s_dx_test, s_dy_test, lambda_reg_test)

        grad_error = check_grad(func, grad, z_flat_test)
        print(f"Gradient check error: {grad_error}")

        if grad_error > self.gradient_accuracy_threshold:
            raise ValueError("Error in gradient computation is too high, check implementation!")
        else:
            print("Gradient computation seems fine, proceeding with integration.")
