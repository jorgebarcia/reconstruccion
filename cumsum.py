...
class Reconstruccion:
    ...
    def integracion_variacional(self, c=1, d=1, lambda_smooth=1e-4, eps=1e-5):
        i_a = self.datos.img_dict['right'].astype(np.float32)
        i_b = self.datos.img_dict['left'].astype(np.float32)
        i_c = self.datos.img_dict['top'].astype(np.float32)
        i_d = self.datos.img_dict['bottom'].astype(np.float32)
        ...
        def energy_function(Z, s_dx, s_dy, lambda_smooth):
            Z = Z.reshape(s_dx.shape)
            dz_dx = np.gradient(Z, axis=1)
            dz_dy = np.gradient(Z, axis=0)
            smooth_term = lambda_smooth * (np.sum(np.gradient(Z, axis=0) ** 2) + np.sum(np.gradient(Z, axis=1) ** 2))
            data_term = np.sum((dz_dx - s_dx) ** 2) + np.sum((dz_dy - s_dy) ** 2)
            return data_term + smooth_term

        Z_init = np.zeros_like(s_dx)
        result = minimize(energy_function, Z_init.ravel(), args=(s_dx, s_dy, lambda_smooth), method='L-BFGS-B')
    ...
    def integracion_variacional_por_bloques(self, c=1, d=1, lambda_smooth=1e-4, eps=1e-5, block_size=100):
        i_a = self.datos.img_dict['right'].astype(np.float32)
        i_b = self.datos.img_dict['left'].astype(np.float32)
        i_c = self.datos.img_dict['top'].astype(np.float32)
        i_d = self.datos.img_dict['bottom'].astype(np.float32)
        ...
        Z = np.zeros_like(s_dx)

        for i in range(0, s_dx.shape[0], block_size):
            for j in range(0, s_dx.shape[1], block_size):
                block_s_dx = s_dx[i:i + block_size, j:j + block_size]
                block_s_dy = s_dy[i:i + block_size, j:j + block_size]

                def energy_function(Z_block, s_dx_block, s_dy_block, lambda_smooth):
                    Z_block = Z_block.reshape(block_s_dx.shape)
                    dz_dx = np.gradient(Z_block, axis=1)
                    dz_dy = np.gradient(Z_block, axis=0)
                    smooth_term = lambda_smooth * (
                                np.sum(np.gradient(Z_block, axis=0) ** 2) + np.sum(np.gradient(Z_block, axis=1) ** 2))
                    data_term = np.sum((dz_dx - s_dx_block) ** 2) + np.sum((dz_dy - s_dy_block) ** 2)
                    return data_term + smooth_term

                Z_block_init = np.zeros_like(block_s_dx)
                result = minimize(energy_function, Z_block_init.ravel(), args=(block_s_dx, block_s_dy, lambda_smooth),
                                  method='L-BFGS-B')
                Z[i:i + block_size, j:j + block_size] = result.x.reshape(block_s_dx.shape)
